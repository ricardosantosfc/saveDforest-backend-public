...
//req.body.next_scene = scene that comes after sam has been succesefuly recieved by backend, 
...
exports.sam_post = [requireAuth, async function(req, res,next){
    ...
    if (
        ...
        typeof req.body.next_scene !== 'number' ||
...
exports.update_score_sam = [requireAuth, async function(req, res,next){
    ...
    if (typeof req.body.next_scene !== 'number' || typeof req.body.score !== 'number') {
        return res.status(400).send({ success: false, message: "bad request" });
    }   
        try{
            await saveData.updateOne(
